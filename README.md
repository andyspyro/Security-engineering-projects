...
| Secure software | [Secure Lo Fi Study Cafe](secure-lofi-study-cafe/README.md) | Built and self-hosted a Node.js/Express/Socket.IO application with bcrypt authentication, RBAC, CSRF protection, SQLite WAL persistence, hardened systemd service controls, UFW/reverse-proxy guidance, verified backups, indexed SQL security telemetry, forensic queries, and an admin investigation console | `server.js`, `database.js`, `schema.sql`, `sql/security-forensics.sql`, v4.0 self-hosted security engineering report |
