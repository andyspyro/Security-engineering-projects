...
| Secure software | [Secure Lo Fi Study Cafe](secure-lofi-study-cafe/README.md) | Built a Node.js/Express/Socket.IO application with bcrypt authentication, session regeneration, RBAC, CSRF protection, secure uploads, Turso/libSQL shared persistence, indexed SQL security telemetry, forensic queries, and an admin investigation console | `server.js`, `database.js`, `schema.sql`, `sql/security-forensics.sql`, v3.2 free shared-backend security report |
